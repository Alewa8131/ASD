...
TEST(PolynomTest, Output_ParseCombine) {
    Polynom p("x+x+x");

    std::ostringstream oss;
    oss << p;

    EXPECT_EQ(oss.str(), "3x");
}

TEST(PolynomTest, Output_RemoveZeroMiddle) {
    Polynom p("x+y-y");
...
TEST(PolynomTest, ComplexExpression) {
    ...
    std::ostringstream oss;
    oss << r;
    EXPECT_EQ(oss.str(), "x^4+x^3+x^2y+xy");
}
